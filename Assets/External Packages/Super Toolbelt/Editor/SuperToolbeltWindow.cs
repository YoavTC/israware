...
namespace External_Packages.Super_Toolbelt.Editor
{
    public class SuperToolbeltWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            float buttonHeight = position.height / buttonsCount;
            ...
            for (int i = 0; i < buttonsCount; i++)
            {
                ...
                if (GUILayout.Button(BUTTONS[i], GUILayout.Width(position.width), GUILayout.Height(buttonHeight)))
                {
                    int buttonIndex = i;
        ...
        {
            EditorSceneManager.SaveScene(SceneManager.GetSceneByName(sceneA));
